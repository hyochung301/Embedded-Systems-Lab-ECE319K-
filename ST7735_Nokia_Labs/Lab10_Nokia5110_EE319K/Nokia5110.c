...
// Outputs: none
void Nokia5110_PlotClear(int32_t ymin, int32_t ymax){
unsigned long i;
  for(i=0; i<SIZE; i++){
...
// Outputs: none
void Nokia5110_PlotPoint(int32_t y){
long i,j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
  // X goes from 0 to 83
...
// Inputs: none
// Outputs: none
void Nokia5110_ShowPlot(void){int i;
  Nokia5110_SetCursor(0, 1);
  for(i=0; i<SIZE; i=i+1){
    writedata(PlotImage[i]);
  }
}
